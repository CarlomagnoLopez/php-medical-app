...
// {
    $sql = "SELECT `id_organization` as 'mcp-1-sk', `web_site` as website, `organization` as orgname, `secret_key` as secretcode, `contact_email` as contactEmail, `contact_name` as contactName, `phone_number` as phoneNumber, `tax_number` as taxNumber FROM `gr_organizations` WHERE deleted = 0";
    try {
        $response = array();
        // $log = array();
        $stmt = $db->prepare($sql);
